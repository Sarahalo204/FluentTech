...
function ChatInterface() {
  ...
  const [messages, setMessages] = useState([
    {
      id: 'welcome',
      role: 'assistant',
      text: 'Hi there! Ask me anything about the FluentTech curriculum or language learning path.',
      timestamp: '09:00',
    },
    {
      id: 'mock-user-1',
      role: 'user',
      text: 'Can you help me practice a business email?',
      timestamp: '09:02',
    },
    {
      id: 'mock-assistant-1',
      role: 'assistant',
      text: 'Absolutely. Start with a polite greeting, keep your sentences concise, and highlight the action you need clearly.',
      timestamp: '09:03',
      scores: {
        grammar: 88,
        vocabulary: 82,
    ...
    },
    {
      id: 'mock-user-2',
      role: 'user',
      text: 'How would I ask for feedback on my lesson plan?',
      timestamp: '09:05',
    },
    {
      id: 'mock-assistant-2',
      role: 'assistant',
      text: 'You can say: "Could you review my lesson plan and share suggestions for improvement?" This keeps the request clear and polite.',
      timestamp: '09:06',
    },
  ]);
  ...
  const mutation = useMutation(
    async (question) => {
      const response = await axios.post(chatEndpoint, { question });
    ...
    },
    {
      onSuccess: (data, question) => {
        const assistantMessage = {
          ...
          role: 'assistant',
          text: data.answer || 'I could not generate an answer right now. Please try again.',
      ...
      onError: (error) => {
        setMessages((current) => [
          ...
          { id: `user-${Date.now()}`, role: 'user', text: input, timestamp: formatTime() },
          {
            ...
            role: 'assistant',
            text: error?.response?.data?.message || 'There was an error fetching the answer. Please try again.',
            timestamp: formatTime(),
          },
        ]);
        setInput('');
